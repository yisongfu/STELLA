...
def generate_data(args: argparse.Namespace):
    ...
    # fill the nan
    df = df.fillna(0)
    df.index = pd.to_datetime(df["date"].values, format="%Y/%m/%d").to_numpy()
    df = df[df.columns[1:]]
    ...
    if add_day_of_year:
        ...
        doy = (df.index.month - 1) / 12 # df.index.month starts from 1. We need to minus 1 to make it start from 0.
...
if __name__ == "__main__":
    ...
    STEPS_PER_DAY = 1            # every 1 day

    DATASET_NAME = "ChinaTemp"      # sampling frequency: every day
